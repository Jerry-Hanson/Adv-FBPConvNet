...
def process_one_image(model, data, device):
    ...
    noise  = model.noise_module(data)[0][0].cpu().detach().numpy()
...
if __name__ == "__main__":
    ...
    output_folder = "/home/yaoqiulei/RobustMLProject/results/FBPConvNet_Adv-180-view"
    state_dict = torch.load("/data/DATA/yaoqiulei/RobustMLProject/FBPConvNetAdv/ckpt/mayo2016-180view_FBPCONVNet_Adv_FBPConvAdv-2-3-0.00007_2023-12-11-12-41/150_2023-12-11-17-30.pt")
    ...
    for input_ in input_list:
        ...
        output, noise = process_one_image(model, X, 'cuda')
        
        # 将output, raw_image, difference_map保存到指定目录下
        tiff.imwrite(os.path.join(output_folder, name + "_FBPConvNet_Adv_output.tif"), output)
        tiff.imwrite(os.path.join(output_folder, name + "_raw_image.tif"), raw_image.astype(np.float32))
        tiff.imwrite(os.path.join(output_folder, name + "_FBPConvNet_Adv_difference.tif"), (raw_image - output).astype(np.float32))
        tiff.imwrite(os.path.join(output_folder, name + "_FBPConvNet_Adv_noise.tif"), noise)
